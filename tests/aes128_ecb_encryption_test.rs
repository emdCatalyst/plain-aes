...
#[test]
fn encrypt_aes128_ecb_u8_block_1() {
    let key: &[u8] = &[
    ...
    let result = encrypt(plain, CipherVersion::Aes128(key, ModeOfOperation::ECB));
    assert!(cipher.iter().eq(result.unwrap().iter()))
}
#[test]
fn encrypt_aes128_ecb_u8_block_2() {
    let key: &[u8] = &[
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f,
        0x3c,
    ];
...
#[test]
fn encrypt_aes128_ecb_u8_block_3() {
    let key: &[u8] = &[
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f,
        0x3c,
    ];
...
#[test]
fn encrypt_aes128_ecb_u8_block_4() {
    let key: &[u8] = &[
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f,
        0x3c,
    ];
...
fn encrypt_aes128_ecb_empty_string_empty_u8() {
    ...
    let result = encrypt(plain, CipherVersion::Aes128(key, ModeOfOperation::ECB));
    let result2 = encrypt(plain2, CipherVersion::Aes128(key, ModeOfOperation::ECB));
    assert_eq!(result.unwrap_err(), OperationError::EmptyTarget);
    assert_eq!(result2.unwrap_err(), OperationError::EmptyTarget);
}
#[test]
fn encrypt_aes128_ecb_key_expansion_failure() {
    let key = [
